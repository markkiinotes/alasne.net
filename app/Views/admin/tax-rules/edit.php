...
<div class="tax-form-page">
    <header class="tax-form-header">
        <div>
            ...
            <p>
                Update
                <strong><?= $escape($taxRule['name']) ?></strong>
                for
    ...
    <section class="tax-form-help">
        <h2>Historical orders remain unchanged</h2>

        <p>
            Saving this rule affects only future checkouts.
            Every existing order retains the tax rate and
            destination snapshot that was applied when it was placed.
        </p>
    </section>
    ...
    <section class="tax-form-panel">
        <form
            method="POST"
            action="/admin/stores/<?= $escape($store['id']) ?>/tax-rules/<?= $escape($taxRule['id']) ?>"
        >
            <input
                type="hidden"
                name="_csrf_token"
